...
- 全量 Studio artifact 默认链路：Study Guide/report、quiz、flashcards、mind map 和 Audio Overview 都是默认要求。前四类 artifact 应在本轮 session 内生成、下载并落到 `notebooklm/artifacts/`；Audio Overview 只在远端尝试发起一次，且显式使用 `--language zh-CN` 生成中文音频。若创建受阻或 rate limited，一次失败即进入 health warning，不在同一轮反复重试。
...
- 默认正式 Studio artifacts 包含 Study Guide/report、quiz、flashcards、mind map 和 Audio Overview。非音频 artifacts 必须生成并下载落盘；Audio Overview 对当前 session 只尝试发起一次，命令显式传 `--language zh-CN`。若创建成功并返回 artifact id，记录 `requested` / `in_progress` 并进入 audio-index；若创建受阻或 rate limited，写 `audio_create_blocked` health warning，不反复重试、不写 audio-index。不等待 completed、不公开 notebook、不写 share URL。后续新 session 开始时才批量补查旧 pending audio，完成后再公开 notebook link access 并写回 share URL。
...
- `notebooklm/` 下的结构化输出。
- `notebooklm/artifacts/` 下的正式 NotebookLM Studio artifacts：`report-study-guide.md`、quiz、flashcards、mind map、`artifact-status.json`，以及音频完成后的 share URL metadata。
- `notes/` 下的个人追问和理解。
- 知识卡片式 `synthesis.md`。
