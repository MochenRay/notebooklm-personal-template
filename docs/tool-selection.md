...
当前默认流程已把 Web Fast Research 与 Audio Overview 纳入 CLI-first 主路径：添加主 source 且 ready 后，先基于主 source 生成 research queries，再用 `nlm research start --source web --mode fast` 发现并选择性导入相关来源，最后生成并下载 Audio Overview。video、report、quiz、flashcards、mind map 不再默认生成。
