...
YouTube URL
  ...
  -> agent 生成 Audio Overview 并保存可播放分享链接
  -> 本地 notes + synthesis
  -> topic 建议与索引
...
- 生成正式 NotebookLM Audio Overview，并默认公开 notebook link access 以保存可播放 artifact 链接；不默认生成 video。
- 提取核心结构。
- 形成小型知识拓扑。
- 生成个人追问。
- 写本地沉淀与知识卡片。
...
## 阶段 6：生成 Audio Overview 并保存可播放链接
...
`share:artifacts` 会执行 `nlm share public`、读取 `nlm share status --json` 与 `nlm studio status --json`，并把 `notebooklm.sharing.public_link` 与 `notebooklm.artifacts.audio.share_url` 写回 `source.yaml`。NotebookLM artifact URL 依赖 notebook public link access；因此 Pipeline 默认公开 notebook link access，但这不等于生成网站草稿、同步公开仓或发布社媒。

若要基于全部来源，可不传 `--source-ids`，但默认优先显式传入筛选后的 source ids。`notes/process-log.md` 必须记录 create/status/share 命令、audio artifact id、ready/failed 状态和 share URL。本地 `audio.m4a` 下载只作为后台补档或用户明确要求，不阻断 session 收尾；下载失败时记录到 `source.yaml` 与 `process-log.md`，不要把失败误写成已完成。
...
- `notebooklm/artifacts/` 的 audio share URL 与可选本地下载清单。
